...
FONTFILE = PROJECT_PATH + '/res/STHeitiMedium.ttc'
...
class MonthObject(object):
    ...
    def _add_week(self, x, y, color, weekend_color, upper=False):
        ...
        for d in get_weekdays(self.first_weekday):
            ...
            text_obj = TextObject(weekday_text, x, y, self.week_font, weekend_color
                if d in [5, 6] else color)
            x += w + space_w
            self.items.append(text_obj)
    ...
    def _add_days(self, x, y, color, weekend_color, upper=False):
        ...
        for i, v in enumerate(calendar.Calendar().itermonthdays2(self.year, self.month)):
            ...
            if day:
                text_obj = TextObject(day_str, x, y, self.week_font, weekend_color
                if wd in [5, 6] else color)
                x += w + space_w
                self.items.append(text_obj)
            else:
                x += w + space_w
    ...
    def render(self):
        ...
        for item in self.items:
            self.draw.text((self.x + item.x, self.y + item.y), item.text, font=item.
            font, fill=item.color)
...
def make_hex_calendar(img, year):
    """Make Hex Calendar"""
    image_path = PROJECT_PATH + '/res/924935.jpg'
    add_image(img, image_path)

    month_font = ImageFont.truetype(FONTFILE, 18)
    week_font = ImageFont.truetype(FONTFILE, 15)
    year_font = ImageFont.truetype(FONTFILE, 25)
...
def main():
    ...
    img.save(PROJECT_PATH + '/res/hex_calendar_{}.png'.format(year))
